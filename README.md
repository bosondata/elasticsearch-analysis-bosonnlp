...
####方法一
```markdown
$ bin/plugin -u https://github.com/bosondata/elasticsearch-analysis-bosonnlp/files/185321/elasticsearch-analysis-bosonnlp-1.0.0-beta-plugin.zip -i elasticsearch-analysis-bosonnlp
...
2. 安装插件（二者选一）
  ...
  - 通过 Elasticsearch 的 plugin 加载插件，在 Elasticsearch 根目录执行以下命令即可：
    ...
    ```
    $ bin/plugin --url file:/root/path/to/your/elasticsearch-analysis-bosonnlp-{version}-plugin.zip --install  elasticsearch-analysis-bosonnlp
    ```
    
  - 手动解压`elasticsearch-analysis-bosonnlp-{version}-plugin.zip`到`ES-root/plugins/analysis-bosonnlp/`即可。

###设置

运行 Elasticsearch 之前需要在 config 文件夹中修改`elasticsearch.yml`来定义使用玻森中文分析器，并填写玻森 API_TOKEN，即在该文件结尾处添加：
...
index:
  analysis:
    analyzer:
      bosonnlp:
          type: org.elasticsearch.index.analysis.BosonNLPAnalyzerProvider
...
1. `必须在 API_TOKEN：*PUT YOUR API TOKEN HERE* 中填写给定的玻森数据API_TOKEN`，否则无法使用玻森中文分析器。该 API_TOKEN 是[注册玻森数据账号](http://bosonnlp.com/)所获得。
...
index:
  analysis:
    ...
    tokenizer:
      bosonnlp_tokenizer:
          type: org.elasticsearch.index.analysis.BosonNLPTokenizerFactory
          API_URL: http://api.bosonnlp.com/tag/analysis
          # You MUST give the API_TOKEN value, otherwise it doesn't work
          API_TOKEN: *PUT YOUR API TOKEN HERE*
          # Please uncomment if you want to specify ANY ONE of the following
...
1. `必须在 API_TOKEN：*PUT YOUR API TOKEN HERE* 中填写给定的玻森数据API_TOKEN`，否则无法使用玻森 tokenizer。
